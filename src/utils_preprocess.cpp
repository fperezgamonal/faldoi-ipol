...
Parameters init_params(const std::string& file_params, int step_alg){
    ...
    if (file_params.empty()){
        if (params.verbose)
            std::cout << "No parameters text file provided..." << std::endl;
        ...
        if (params.verbose)
            std::cout << "text file provided (will print inputted parameters)..." << std::endl;
        ...
        if (params.verbose) {
            std::cout << "params.lambda: " << params.lambda << std::endl;
        }
        if (params.lambda <= 0) {
            params.lambda = PAR_DEFAULT_LAMBDA;
            if (params.verbose) fprintf(stderr, "warning: lambda changed to default: %g\n", params.lambda);
        }
        params.theta = std::stof(line, &sz); getline(infile, line);
        if (params.verbose) {
            std::cout << "params.theta: " << params.theta << std::endl;
        }
        if (params.theta <= 0) {
            params.theta = PAR_DEFAULT_THETA;
            if (params.verbose) fprintf(stderr, "warning: "
                                                "theta changed to default:%g\n", params.theta);
        }
        ...
        if (params.beta <= 0) {
            params.beta = PAR_DEFAULT_BETA;
            if (params.verbose) fprintf(stderr, "warning: "
                                         "beta changed to default: %f\n", params.beta);
        }
        params.alpha = std::stof(line, &sz); getline(infile, line);
        if (params.verbose) {
            std::cout << "params.alpha: " << params.tau << std::endl;
        }
        if (params.alpha <= 0) {
            params.alpha = PAR_DEFAULT_ALPHA;
            if (params.verbose) fprintf(stderr, "warning: "
                                         "alpha changed to default: %f\n", params.alpha);
        }
        params.tau_u = std::stof(line, &sz); getline(infile, line);
        if (params.verbose) {
            std::cout << "params.tau_u: " << params.tau_u << std::endl;
        }
        if (params.tau_u <= 0 || params.tau_u > 0.25) {
            params.tau_u = PAR_DEFAULT_TAU_U;
            if (params.verbose) fprintf(stderr, "warning: "
                                                "tau_u changed to default: %g\n", params.tau_u);
        }
        params.tau_eta = std::stof(line, &sz); getline(infile, line);
        if (params.verbose) {
            std::cout << "params.tau_eta: " << params.tau_eta << std::endl;
        }
        if (params.tau_eta <= 0 || params.tau_eta > 0.25) {
            params.tau_eta = PAR_DEFAULT_TAU_ETA;
            if (params.verbose) fprintf(stderr, "warning: "
                                                "tau_eta changed to default: %g\n", params.tau_eta);
        }
        params.tau_chi = std::stof(line, &sz); getline(infile, line);
        if (params.verbose) {
            std::cout << "params.tau_chi: " << params.tau_chi << std::endl;
        }
        if (params.tau_chi <= 0 || params.tau_chi > 0.25) {
            params.tau_chi = PAR_DEFAULT_TAU_CHI;
            if (params.verbose) fprintf(stderr, "warning: "
                                                "tau_chi changed to default: %g\n", params.tau_chi);
        }
        params.mu = std::stof(line, &sz); getline(infile, line);
        if (params.verbose) {
            std::cout << "params.mu: " << params.mu << std::endl;
        }
        if (params.mu <= 0) {
            params.mu = PAR_DEFAULT_MU;
            if (params.verbose) fprintf(stderr, "warning: "
                                         "mu changed to default: %f\n", params.mu);
        }
        infile.close();
    }
    return params;
