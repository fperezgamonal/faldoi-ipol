...
//#pragma omp parallel for reduction(+:err_D) //commented because it did not work? (check)
  // the above sums the error computed by each thread, finishing before than if only one
  // of them computed all the iterations
  for (int l = ij; l < ej; l++)
  for (int k = ii; k < ei; k++)
  {
    const int i = l*w + k;      
...
//#pragma omp parallel for schedule(dynamic,1) collapse(2)
      ...
      nltvcsad_getP(v1, v2, div_p, div_q, theta, tau,
