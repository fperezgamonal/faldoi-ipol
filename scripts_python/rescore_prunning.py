...
def score_from_autocorr(img0, img1, corres):
  ...
  sigma_matrix = 1.0 # for the integration gaussian filter
  ...
  for pos0, pos1, score in corres:
    ...
    res.append((pos0,pos1,sqrt( max(0,small_eigen[p0]))))
  return res


def confidence_values(i0, i1, corres, dest_of):
  img0 = array(Image.open(i0).convert('RGB'))
  img1 = array(Image.open(i1).convert('RGB'))
  match = corres; 
